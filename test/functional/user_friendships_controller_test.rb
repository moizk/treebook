...
class UserFriendshipsControllerTest < ActionController::TestCase
	...
	context "#index" do
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:moiz), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:moiz), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
	...
	context "#new" do
		...
		context "when logged in" do
			...
			should "display the friend's name" do
				get :new, friend_id: users(:alex)
				assert_match /#{users(:alex).full_name}/, response.body
			end
			...
			should "assign a new user friendship to the correct user" do
				get :new, friend_id: users(:alex)
				assert_equal users(:alex), assigns(:user_friendship).friend
			end

			should "assign a new user friendship to the currently logged in user" do
				get :new, friend_id: users(:alex)
				assert_equal users(:moiz), assigns(:user_friendship).user
			end

			should "return 404 status is no friend is found" do
			...
			should "ask if you really want to friend the user" do
				get :new, friend_id: users(:alex)
				assert_match /Do you really want to friend #{users(:alex).full_name}?/, response.body
			end
		end
	end
		...
		context "when logged in" do
			...
			context "with a valid friend_id" do
				...
				should "create a friendship" do
					assert users(:moiz).pending_friends.include?(users(:mike))
				end
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:pending_user_friendship, user: users(:moiz), friend: @friend)
				create(:pending_user_friendship, friend: users(:moiz), user: @friend)
				sign_in users(:moiz)
	...
	context "#edit" do
		...
		context "when logged in" do
			setup do
				@user_friendship = create(:pending_user_friendship, user: users(:moiz))
				sign_in users(:moiz)
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:moiz))
        create(:accepted_user_friendship, friend: users(:moiz), user: @friend)
